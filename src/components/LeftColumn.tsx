  ...
  return (
    <section className="flex flex-col relative z-10">
      <div className="flex items-center justify-between mb-5 px-1">
        <h2 className="text-base sm:text-lg font-bold uppercase tracking-widest text-slate-500">用户输入区</h2>
        {isRecording && (
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-[2.5rem] border border-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-10 flex-1 flex flex-col relative overflow-hidden transition-all hover:shadow-[0_8px_40px_rgb(0,0,0,0.08)]">
        ...
        <button
          ...
          className={`w-full flex items-center justify-center gap-3 py-5 rounded-2xl border-2 transition-all duration-300 group mb-8 relative overflow-hidden text-lg ${
            isRecording 
        ...
        <div className="flex-1 min-h-[18rem] bg-slate-50 rounded-2xl p-7 border border-slate-100 relative flex flex-col">
          {isRecording && text ? (
            ...
            <textarea
              ...
              className="w-full h-full bg-transparent focus:outline-none resize-none text-xl leading-relaxed text-slate-600 placeholder:text-slate-400"
            />
          )}
          {isRecording && (
            <span className="absolute bottom-5 right-5 text-sm text-slate-400">点击按钮停止并确认</span>
          )}
        </div>
      </div>
    </section>
